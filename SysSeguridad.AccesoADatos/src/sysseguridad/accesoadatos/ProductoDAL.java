...
public class ProductoDAL {  // Clase para poder realizar consulta de Insertar, modificar, eliminar, obtener datos de la tabla Producto
    ...
    public static int crear(Producto pProducto) throws Exception {
        ...
        String sql;
            ...
            sql = "INSERT INTO Producto(IdProveedor,IdCategoria,Nombre,Descripcion,Precio,Existencia) VALUES(?,?,?,?,?,?)";
...
// Metodo para obtener todos los registro de la tabla de Producto
    ...
    static void querySelect(Producto pProducto, ComunDB.UtilQuery pUtilQuery) throws SQLException {
     ...
     // Metodo para obtener todos los registro de la tabla de Provvedor que cumplan con los filtros agregados 
     // a la consulta SELECT de la tabla de Provvedor
    public static ArrayList<Producto> buscar(Producto  pProducto ) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            querySelect(pProducto , utilQuery); // Asignar el filtro a la consulta SELECT de la tabla de Rol 
            sql = utilQuery.getSQL(); 
            sql += agregarOrderBy(pProducto ); // Concatenar a la consulta SELECT de la tabla Rol el ORDER BY por Id
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                utilQuery.setStatement(ps);
                utilQuery.setSQL(null);
                utilQuery.setNumWhere(0); 
                querySelect(pProducto , utilQuery);  // Asignar los parametros al PreparedStatement de la consulta SELECT de la tabla de Rol
                obtenerDatos(ps, Producto ); // Llenar el ArrayList de Rol con las fila que devolvera la consulta SELECT a la tabla de Rol
                ps.close(); // Cerrar el PreparedStatement
            } catch (SQLException ex) {
                throw ex;  // Enviar al siguiente metodo el error al ejecutar PreparedStatement en el caso que suceda
        ...
        return Producto; // Devolver el ArrayList de Rol
    }
    ...
    public static ArrayList<Producto> buscarIncluirProveedor(Producto pProducto) throws Exception {
        ArrayList<Producto> producto = new ArrayList();
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ...
                obtenerDatosIncluirProveedor(ps, producto);// Llenar el ArrayList de Usuario con las fila que devolvera la consulta SELECT a la tabla de Usuario
                ps.close(); // Cerrar el PreparedStatement
            } catch (SQLException ex) {
                throw ex;// Enviar al siguiente metodo el error al ejecutar PreparedStatement en el caso que suceda
            }
        ...
        return producto; // Devolver el ArrayList de Usuario
    }
